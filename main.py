...
def data_three(dirname):
    ...
    ga = RuleBasedFloatingPointGA(dataset=training_set,test_data=test_set,
        mutation_probability=0.018, crossover_probability=0.9, 
...
def data_four(dirname):
    ...
    test_set = data[1::2]
    ga = RuleBasedFloatingPointGA(dataset=training_set,test_data=test_set,
        mutation_probability=0.009, crossover_probability=0.85, 
        population_size=50, rule_count=10, seed_edge_cases=True)
    ga.evolve(epoch=5000)
